        ...
        tryCatch(
          {
            ...
            audio_src <- if (grepl("^http", audio_path())) {
              audio_path()
            } else if (filters_used()) {
              # Filtered audio is already saved to td by filteredAudio()
              paste0("audiodir/", basename(audio_path()))
            } else {
              # Copy local file to td so it can be served via resource path
              dest <- file.path(td, basename(audio_path()))
              if (!file.exists(dest)) file.copy(audio_path(), dest)
              paste0("audiodir/", basename(audio_path()))
            }
